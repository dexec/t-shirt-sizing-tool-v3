...
export const useBucketsStore = defineStore("buckets", () => {
    ...
    function addNewBucket(id: number, before: boolean): Bucket {
        ...
        let lastAddedBucketName;
        let newBucketName;
        if (bucketsAsMap.value.size == 0) newBucketName = 'S'
        else {
            lastAddedBucketName = bucketsAsSortedArray.value.reduce((a, b) => a.id > b.id ? a : b).name
            if (/^\d+$/.test(lastAddedBucketName.slice(-1))) {
                const lastCharAsInt = parseInt(lastAddedBucketName.slice(-1));
                newBucketName = lastAddedBucketName.slice(0, -1) + (lastCharAsInt + 1)
            } else {
                newBucketName = lastAddedBucketName + "1"
            }
        }
