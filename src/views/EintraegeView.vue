...
const columnDefs: ColDef[] = [
  ...
  },
  {
    ...
    valueSetter: (params: any): any => {
      ...
      if (!isNaN(newValue)) {
        eintragContainer.updateAufwandRelativ(params.node.rowIndex, +newValue);
  ...
  },
  {
    ...
    valueSetter: (params: any) => {
      const newValue = params.newValue.replace(",", ".");
      if (!isNaN(newValue)) eintragContainer.updateAufwandAbsolut(params.node.rowIndex, +newValue);
...
const rowData = eintragContainer.eintraege;
  ...
  switch (bezeichnung) {
    ...
    case SummeET.ZWISCHENSUMME: {
      ...
      for (let i = vorigerAbschnittEintraege.length - 1; i >= 1; i--) {
        ...
        } else aufschlag = vorigerAbschnittEintraege[i].aufwandRelativ.toFixed(konfigContainer.nachkommastellen);
      ...
      } else startaufschlag = vorigerAbschnittEintraege[0].aufwandRelativ.toFixed(konfigContainer.nachkommastellen);
      projektkalkulationStore.erklaerungsRechnung += startaufschlag + "%";
      //projektkalkulationStore.erklaerungsRechnung = `Das ergibt <span style=color:lightgreen>${projektkalkulationStore.erklaerungsRechnung}</span> = <span style=color:lightblue>${aktuellerEintrag.vorigerAbschnittAufwandRelativ.toFixed(konfigContainer.nachkommastellen)}%</span>`;
      projektkalkulationStore.erklaerungsRechnung = `<span style=color:lightgreen>${projektkalkulationStore.erklaerungsRechnung}</span> = <span style=color:lightblue>${aktuellerEintrag.vorigerAbschnittAufwandRelativ.toFixed(konfigContainer.nachkommastellen)}%</span>`;
      break;
    }
    default : {
      ...
      let aufwand;
      let aufschlag;
      const Zwischensumme = aktuellerEintrag.basisZwischensumme.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen);
      if (aktuellerEintrag.isAufwandRelativBase) {
        aufwand = aktuellerEintrag.aufwandAbsolut.toFixed(konfigContainer.nachkommastellen);
        aufschlag = aktuellerEintrag.aufwandRelativ;
      } else {
        aufwand = aktuellerEintrag.aufwandAbsolut;
        aufschlag = aktuellerEintrag.aufwandRelativ.toFixed(konfigContainer.nachkommastellen);
      }
...
function erklaereAufwand(bezeichnung: string, rowIndex: number) {
  switch (bezeichnung) {
    case SummeET.STARTSUMME: {
      ...
      //projektkalkulationStore.erklaerungsRechnung = `Das ergibt hier <span style=color:orange>${(rowData[0] as Zwischensumme).zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span>`;
      break;
    }
    case SummeET.ZWISCHENSUMME: {
      const aktuellerEintrag = gridApi.value!.getRowNode(rowIndex + "")!.data as Zwischensumme;
      ...
      for (let i = vorigerAbschnittEintraege.length - 1; i >= 1; i--) {
        let aufwand;
        if (vorigerAbschnittEintraege[i].isAufwandRelativBase) {
          aufwand = vorigerAbschnittEintraege[i].aufwandAbsolut.toFixed(konfigContainer.nachkommastellen);
      ...
      let startaufwand;
      if (vorigerAbschnittEintraege[0].isAufwandRelativBase) {
        startaufwand = vorigerAbschnittEintraege[0].aufwandAbsolut.toFixed(konfigContainer.nachkommastellen);
      } else startaufwand = vorigerAbschnittEintraege[0].aufwandAbsolut;
      projektkalkulationStore.erklaerungsRechnung += +startaufwand;
      //projektkalkulationStore.erklaerungsRechnung = `Das ergibt für den oberen Wert <span style=color:lightgreen>${projektkalkulationStore.erklaerungsRechnung}</span> = <span style=color:lightblue>${aktuellerEintrag.vorigerAbschnittAufwandAbsolut.toFixed(konfigContainer.nachkommastellen)}</span>`;
      projektkalkulationStore.erklaerungsRechnung = `<span style=color:lightgreen>${projektkalkulationStore.erklaerungsRechnung}</span> = <span style=color:lightblue>${aktuellerEintrag.vorigerAbschnittAufwandAbsolut.toFixed(konfigContainer.nachkommastellen)}</span>`;
      //projektkalkulationStore.erklaerungsRechnungZusatz = `Für den unteren Wert ergibt das <span style=color:lightblue>${aktuellerEintrag.vorigerAbschnittAufwandAbsolut.toFixed(konfigContainer.nachkommastellen)}</span> + <span style=color:orange>${vorigerAbschnittEintraege[0].basisZwischensumme.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span> = <span style=color:lightblue>${aktuellerEintrag.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span>`;
      projektkalkulationStore.erklaerungsRechnungZusatz = `<span style=color:orange>${vorigerAbschnittEintraege[0].basisZwischensumme.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span> + <span style=color:lightblue>${aktuellerEintrag.vorigerAbschnittAufwandAbsolut.toFixed(konfigContainer.nachkommastellen)}</span> = <span style=color:lightblue>${aktuellerEintrag.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span>`;
      break;
    }
    case SummeET.ENDSUMME: {
      ...
      for (let i = alleEintraege.length - 1; i >= 0; i--) {
        projektkalkulationStore.erklaerungsRechnung += " + " + alleEintraege[i].toFixed(konfigContainer.nachkommastellen);
      }
      projektkalkulationStore.erklaerungsText = "Die Endsumme ergibt sich aus der Summe aller voriger Aufwände und der Startsumme.";
      //projektkalkulationStore.erklaerungsRechnung = `Das ergibt <span style=color:lightgreen>${projektkalkulationStore.erklaerungsRechnung}</span> = <span style=color:lightblue>${endsumme.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span>`;
      projektkalkulationStore.erklaerungsRechnung = `<span style=color:lightgreen>${projektkalkulationStore.erklaerungsRechnung}</span> = <span style=color:lightblue>${endsumme.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span>`;
      break;
    }
    default : {
      const aktuellerEintrag = gridApi.value!.getRowNode(rowIndex + "")!.data as Aufschlag;
...
function erklaereAnteilAnZwischensumme(bezeichnung: string, rowIndex: number) {
  switch (bezeichnung) {
    ...
    case SummeET.ZWISCHENSUMME: {
      ...
      //projektkalkulationStore.erklaerungsRechnung = `Daraus ergibt sich <span style=color:lightgreen>${aktuellerEintrag.vorigerAbschnittAufwandAbsolut.toFixed(konfigContainer.nachkommastellen)}</span> / <span style=color:orange>${aktuellerEintrag.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span> = <span style=color:lightblue>${aktuellerEintrag.anteilZwischensumme.toFixed(konfigContainer.nachkommastellen)}%</span>`;
      projektkalkulationStore.erklaerungsRechnung = `<span style=color:lightgreen>${aktuellerEintrag.vorigerAbschnittAufwandAbsolut.toFixed(konfigContainer.nachkommastellen)}</span> / <span style=color:orange>${aktuellerEintrag.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span> = <span style=color:lightblue>${aktuellerEintrag.anteilZwischensumme.toFixed(konfigContainer.nachkommastellen)}%</span>`;
      break;
    }
    default: {
      const aktuellerEintrag = gridApi.value!.getRowNode(rowIndex + "")!.data as Aufschlag;
      ...
      //projektkalkulationStore.erklaerungsRechnung = `Das ergibt <span style=color:lightgreen>${aktuellerEintrag.aufwandAbsolut.toFixed(konfigContainer.nachkommastellen)}</span> / <span style=color:orange>${naechsteZwischensumme.toFixed(konfigContainer.nachkommastellen)}</span> = <span style=color:lightblue>${aktuellerEintrag.anteilZwischensumme.toFixed(konfigContainer.nachkommastellen)}%</span>`;
      projektkalkulationStore.erklaerungsRechnung = `<span style=color:lightgreen>${aktuellerEintrag.aufwandAbsolut.toFixed(konfigContainer.nachkommastellen)}</span> / <span style=color:orange>${naechsteZwischensumme.toFixed(konfigContainer.nachkommastellen)}</span> = <span style=color:lightblue>${aktuellerEintrag.anteilZwischensumme.toFixed(konfigContainer.nachkommastellen)}%</span>`;
    }
  }
}
...
function erklaereAnteilAnGesamtprojekt(bezeichnung: string, rowIndex: number) {
  ...
  switch (bezeichnung) {
    ...
    case SummeET.ZWISCHENSUMME: {
      ...
      //projektkalkulationStore.erklaerungsRechnung = `Daraus ergibt sich <span style=color:lightgreen>${aktuellerEintrag.vorigerAbschnittAufwandAbsolut.toFixed(konfigContainer.nachkommastellen)}</span> / <span style=color:orange>${endsumme.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span> = <span style=color:lightblue>${aktuellerEintrag.anteilGesamtprojekt.toFixed(konfigContainer.nachkommastellen)}%</span>`;
      projektkalkulationStore.erklaerungsRechnung = `<span style=color:lightgreen>${aktuellerEintrag.vorigerAbschnittAufwandAbsolut.toFixed(konfigContainer.nachkommastellen)}</span> / <span style=color:orange>${endsumme.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span> = <span style=color:lightblue>${aktuellerEintrag.anteilGesamtprojekt.toFixed(konfigContainer.nachkommastellen)}%</span>`;
      break;
    }
    default: {
      const aktuellerEintrag = gridApi.value!.getRowNode(rowIndex + "")!.data as Aufschlag;
      ...
      //projektkalkulationStore.erklaerungsRechnung = `Das ergibt <span style=color:lightgreen>${aktuellerEintrag.aufwandAbsolut.toFixed(konfigContainer.nachkommastellen)}</span> / <span style=color:orange>${endsumme.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span> = <span style=color:lightblue>${aktuellerEintrag.anteilGesamtprojekt.toFixed(konfigContainer.nachkommastellen)}%</span>`;
      projektkalkulationStore.erklaerungsRechnung = `<span style=color:lightgreen>${aktuellerEintrag.aufwandAbsolut.toFixed(konfigContainer.nachkommastellen)}</span> / <span style=color:orange>${endsumme.zwischensummeAufwand.toFixed(konfigContainer.nachkommastellen)}</span> = <span style=color:lightblue>${aktuellerEintrag.anteilGesamtprojekt.toFixed(konfigContainer.nachkommastellen)}%</span>`;
    }
  }
}
          ...
          if (shift || ctrl) {
            ...
            if (![ColumnET.ZWISCHENSUMME, ColumnET.GESAMTPROJEKT].includes(colKey) && ![SummeET.ZWISCHENSUMME, SummeET.STARTSUMME, SummeET.ENDSUMME].includes(e.data.bezeichnung)) {
              if (colKey == ColumnET.AUFWAND) {
                eintragContainer.updateAufwandAbsolut(e.rowIndex, 0);
              } else if (colKey == ColumnET.AUFSCHLAG) {
                eintragContainer.updateAufwandRelativ(e.rowIndex, 0);
              }
...
function startEditingCell(e: any, colKey: string) {
  if ([ColumnET.BEZEICHNUNG, ColumnET.AUFSCHLAG, ColumnET.AUFWAND].includes(colKey as ColumnET) && !Object.values(SummeET).includes(e.data.bezeichnung)) {
    ...
    if (colKey as ColumnET == ColumnET.AUFSCHLAG && !e.data.isAufwandRelativBase) {
      e.data.aufwandRelativ = parseFloat(e.data.aufwandRelativ.toFixed(konfigContainer.nachkommastellen));
    } else if (colKey as ColumnET == ColumnET.AUFWAND && e.data.isAufwandRelativBase) {
      e.data.aufwandAbsolut = parseFloat(e.data.aufwandAbsolut.toFixed(konfigContainer.nachkommastellen));
    }
...
function stopEiditingAndSetFocus(cancel: boolean, rowIndex: number, colKey: string) {
  ...
  nextTick(() => eintragContainer.berechne());
  columnDefs.forEach(column => column.editable = false);
  gridApi.value!.setGridOption("columnDefs", columnDefs);
...
function aufschlagErstellen() {
  if (gridApi.value!.getSelectedRows()[0] && gridApi.value!.getSelectedRows()[0].bezeichnung !== SummeET.ENDSUMME) {
    ...
    eintragContainer.addNewEintrag(rowIndexSelectedRow);
    const focusedCell = gridApi.value!.getFocusedCell();
    let colKey = "";
    if (focusedCell != null) {
...
function zwischensummeErstellen() {
  ...
  if (gridApi.value!.getSelectedRows()[0] != null && focusedCell != null) {
    ...
    if (!Object.values(SummeET).includes(bezeichnungSelectedRow) && ![SummeET.ZWISCHENSUMME as string, SummeET.STARTSUMME as string].includes(bezeichnungSelectedRowUnder)) {
      eintragContainer.addNewZwischensumme(focusedCell.rowIndex);
...
function eintragEntfernen() {
  ...
  if (gridApi.value!.getSelectedRows()[0] != null && focusedCell != null) {
    ...
    if (![SummeET.STARTSUMME, SummeET.ENDSUMME].includes(bezeichnungSelectedRow)) {
      eintragContainer.deleteEintrag(focusedRowIndex);
...
function moveZeileUp() {
  if (gridApi.value!.getSelectedRows()[0]) {
    ...
    if (![SummeET.STARTSUMME, SummeET.ENDSUMME].includes(bezeichnungSelectedRow)) {
      ...
      const focusedRowIndex = focusedCell!.rowIndex;
      const focusedRowColKey = focusedCell!.column;
      eintragContainer.moveUp(gridApi.value!.getFocusedCell()!.rowIndex);
      refreshTable(focusedRowColKey, focusedRowIndex - 1);
    }
  }
}
...
function moveZeileDown() {
  if (gridApi.value!.getSelectedRows()[0]) {
    ...
    if (bezeichnungSelectedRowUnder !== SummeET.ENDSUMME && ![SummeET.STARTSUMME, SummeET.ENDSUMME].includes(bezeichnungSelectedRow)) {
      const focusedRowColKey = focusedCell!.column;
      eintragContainer.moveDown(focusedRowIndex);
      refreshTable(focusedRowColKey, focusedRowIndex + 1);
    }
  }
}
...
function refreshTable(colKey?: Column | string, rowIndex?: number) {
  nextTick(() => {
    gridApi.value!.setGridOption("rowData", eintragContainer.eintraege);
    gridApi.value!.forEachNode(function(node) {
      if (node.data.bezeichnung === SummeET.ZWISCHENSUMME) node.setRowHeight(80);
    });
    gridApi.value!.onRowHeightChanged();
